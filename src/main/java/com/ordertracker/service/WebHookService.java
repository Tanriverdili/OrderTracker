...
public class WebHookService {
    ...
    public void handleShipmentWebhook(
        ...
        try {
            ...
            order.setStatus(status);
            orderRepository.save(order);
            log.info("Shipment updated. OrderId={}, Status={}",
                    orderId, status);
            emailService.sendOrderStatusEmail(
                    order.getUser().getEmail(),
                    status.name()
            ...
            log.error("Shipment webhook failed for order {}", orderId, e);
            webhookLogEntity.setStatus("FAILED");
            webhookLogEntity.setResponseMessage(
                    e.getMessage()
            );
    ...
    public void retryFailedWebhooks() {
        List<WebhookLogEntity> failedLogs = webHookLogRepository.findByStatus("FAILED");
